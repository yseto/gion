...
<template>
  <div class="navbar navbar-default navbar-fixed-top" role="navigation">
    <div class="container">
      <div class="navbar-header">
        <button type="button" class="navbar-toggle" v-on:click="$root.navbar">
      ...
      <div class="navbar-collapse collapse" v-bind:class="{ show: $root.navbarState }">
        <ul class="nav navbar-nav" v-for="item in items">
          <li v-bind:class="{ active: $root.currentRoute === item.route }">
            <a v-on:click="$root.go(item.route)"><span :class=item.icon></span> {{ item.caption }}</a>
          </li>
        </ul>
        <ul class="nav navbar-nav navbar-right">
          ...
          <li class="hidden-sm">
            <a style="cursor:pointer;" v-on:click="$root.helpModal=true">
  ...
  <div class="modal show" id="helpModal" tabindex="-1" v-if="$root.helpModal">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button class="btn btn-default" v-on:click="$root.helpModal=false">Close</button>
        </div>
      </div>
    </div>
  </div>
  
  <div class="modal show" id="errorModal" tabindex="-1" v-if="$root.error">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
        ...
        <div class="modal-footer">
          <button class="btn btn-default" v-on:click.prevent="$root.error=false">Close</button>
        </div>
      </div>
    </div>
  </div>
